...
class _DriverWaterTestScreenState extends ConsumerState<DriverWaterTestScreen> {
  ...
  static const List<Map<String, dynamic>> _htVolumes = [
    {
      'id': 'ht_sm',
      'label': 'Small (2-3 person)',
      'desc': 'Compact, plug & play',
      'liters': 700,
    },
    {
      'id': 'ht_md',
      'label': 'Medium (4-5 person)',
      'desc': 'Most common family spa',
      'liters': 1200,
    },
    {
      'id': 'ht_lg',
      'label': 'Large (6-8 person)',
      'desc': 'Party/family size',
      'liters': 1800,
    },
    {
      'id': 'ht_xl',
      'label': 'XL (8+ / Swim Spa)',
      'desc': 'Oversized or swim spa',
      'liters': 2500,
    },
  ];
  ...
  static const List<Map<String, dynamic>> _poolVolumes = [
    {
      'id': 'pl_sm',
      'label': 'Small Above-Ground',
      'desc': '12-15 ft round, 4 ft deep',
      'liters': 15000,
    },
    {
      'id': 'pl_md',
      'label': 'Medium',
      'desc': '15×30 ft, avg 5 ft deep',
      'liters': 40000,
    },
    {
      'id': 'pl_lg',
      'label': 'Large In-Ground',
      'desc': '20×40 ft, avg 5.5 ft deep',
      'liters': 60000,
    },
    {
      'id': 'pl_xl',
      'label': 'XL / Commercial',
      'desc': '25×50 ft+ or commercial',
      'liters': 80000,
    },
  ];
    ...
    if (_waterType == 'pool') {
      ...
      if (vol > 0) {
        ...
        for (final v in _poolVolumes) {
          if ((v['liters'] as int) == vol) {
            _volumeId = v['id'] as String;
            break;
          }
        }
        _volumeId ??= 'custom';
      ...
      if (vol > 0) {
        ...
        for (final v in _htVolumes) {
          if ((v['liters'] as int).toDouble() == vol) {
            _volumeId = v['id'] as String;
            break;
          }
        }
  ...
  String get _poolType => _waterType == 'pool'
      ? 'pool'
      : _waterType == 'both'
      ? 'both'
      : 'hottub';
  ...
  String _rangeText(String key) {
    final ranges = _poolType == 'hottub'
        ? WaterTestRanges.hotTub
        : WaterTestRanges.pool; // 'both' uses pool ranges
    final r = ranges[key];
    if (r == null) return '';
    final mn = r['min']!;
    final mx = r['max']!;
  ...
  void _generateCrate() {
    ...
    final hasAnyValue = _ctrls.values.any(
      (c) => c.text.trim().isNotEmpty && double.tryParse(c.text.trim()) != null,
    );
    final hasAnyVisualCheck =
        _foam || _cloudy || _filterDirty || _scale || _flush || _algae;
  ...
  Widget build(BuildContext context) {
    ...
    final stepSubtitles = [
      'What are we testing?',
      _waterType == 'both'
          ? '♨️🏊 Both · ${_volumeLiters?.toInt() ?? '?'}L'
          : _waterType == 'hot_tub'
          ? '♨️ Hot Tub · ${_volumeLiters?.toInt() ?? '?'}L'
          : '🏊 Pool · ${_volumeLiters?.toInt() ?? '?'}L',
      '${widget.delivery.safeCustomerName} · ${_poolType == 'both'
          ? 'Pool + Hot Tub'
          : _poolType == 'hottub'
          ? 'Hot tub'
          : 'Pool'}',
        ...
        children: [
          ...
          Row(
            children: [
              ...
              SizedBox(width: 10.w),
              Expanded(
                child: _typeCard('pool', '🏊', 'Pool', 'Chlorine or Salt'),
              ),
            ],
          ),
          SizedBox(height: 8.h),
          ...
          if (_waterType == 'both') ...[
            _infoBox(
              'Select the volume for the body of water you are testing now',
            ),
            SizedBox(height: 12.h),
            _sectionLabel('🏊 Pool'),
            SizedBox(height: 4.h),
            ...
            _infoBox(
              'Volume determines product SIZE — bigger ${_waterType == "hot_tub" ? "spa" : "pool"} = bigger containers',
            ),
            SizedBox(height: 8.h),
            ...(_waterType == 'hot_tub' ? _htVolumes : _poolVolumes).map(
              (v) => _volumeCard(v),
            ),
          ],
          _customVolumeCard(),
  ...
  Widget _typeCard(String value, String emoji, String label, String desc) {
    ...
    return GestureDetector(
      onTap: () => setState(() {
        ...
        _volumeId = null;
        _volumeLiters = null;
        _customVolCtrl.clear();
        _lastDrain = null;
        _sanitizerType = value == 'hot_tub'
            ? 'bromine'
            : (value == 'both' ? 'chlorine' : 'chlorine');
      }),
      child: Container(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: selected ? const Color(0xFF2C6E7A) : Colors.grey.shade300,
            width: selected ? 2 : 1.5,
          ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: selected ? const Color(0xFFC8A84E) : Colors.grey.shade300,
            width: selected ? 2 : 1.5,
          ),
          borderRadius: BorderRadius.circular(10.r),
        ),
            ...
            if (selected)
              SizedBox(
                ...
                child: TextField(
                  ...
                  onChanged: (val) =>
                      setState(() => _volumeLiters = double.tryParse(val)),
                ),
              )
            else
  ...
  Widget _drainChip(
    ...
    return GestureDetector(
      onTap: () => setState(() => _lastDrain = selected ? null : value),
      child: Container(
        width: fullWidth ? double.infinity : null,
        padding: EdgeInsets.symmetric(horizontal: 16.w, vertical: 10.h),
        decoration: BoxDecoration(
          color: selected
              ? (isWarning ? const Color(0xFFFAEAED) : const Color(0xFFE8F4F6))
        ...
        ? [
            {
              ...
              'label': 'Bromine',
              'desc':
                  'Most popular for hot tubs — low odour, stable in warm water',
            },
            {
              'value': 'chlorine',
              'label': 'Chlorine',
              'desc': 'Budget-friendly — available as tablets or granules',
            },
          ]
        : _waterType == 'both'
        ? [
            ...
            },
            {
              'value': 'chlorine',
              'label': 'Chlorine',
              'desc': 'Standard sanitizer — works for pool & hot tub',
            },
            {
              'value': 'salt',
              'label': 'Salt System',
              'desc': 'Salt chlorine generator — typically for pools',
            },
          ]
        : [
            {
              'value': 'chlorine',
              'label': 'Chlorine',
              'desc': 'Standard pool sanitizer — tablets in skimmer or floater',
            },
            {
              'value': 'salt',
              'label': 'Salt System',
              'desc': 'Salt chlorine generator — still needs monitoring',
            },
          ];
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 4.h),
          Text(
            'Determines which products AutoCrate can recommend',
  ...
  Widget _buildStep2WaterTest() {
    ...
    final visibleParams = allParams.where((p) {
      ...
      if (key == 'cyanuricAcid' && _waterType == 'hot_tub')
        return false; // show CYA for pool & both
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Ideal ranges shown for ${_poolType == "hottub"
                        ? "hot tub"
                        : _poolType == "both"
                        ? "pool + hot tub"
                        : "pool"}. Red border = out of range.',
                    style: GoogleFonts.inter(
                      fontSize: 10.sp,
                      color: const Color(0xFF2C6E7A),
                      fontWeight: FontWeight.w600,
      ...
      child: Column(
        ...
        children: [
          ...
          _visualToggle(
            '☁️',
            'Cloudy Water',
            isPool ? 'Triggers: Quick Clear' : 'Triggers: Spa Shock',
            _cloudy,
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 2.h),
                  Text(
                    'No issues — tap to reset all selections',
                    style: GoogleFonts.inter(
                      fontSize: 10.sp,
                      color: Colors.grey,
                    ),
          ...
          _visualToggle(
            ...
            'Scale Buildup',
            isPool
                ? 'Triggers: Stain Preventer'
                : 'Triggers: Stain&Scale + Cover Cleaner',
            _scale,
            (v) => setState(() => _scale = v),
          ),
          _visualToggle(
            ...
            'Triggers: Floating Dispenser starter kit',
            _isFirstVisit,
            (v) => setState(() => _isFirstVisit = v),
          ),
          GestureDetector(
            onTap: () => setState(() {
              _foam = false;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: value ? const Color(0xFF9C2B3A) : Colors.grey.shade300,
            width: value ? 2 : 1.5,
          ),
          borderRadius: BorderRadius.circular(12.r),
        ),
